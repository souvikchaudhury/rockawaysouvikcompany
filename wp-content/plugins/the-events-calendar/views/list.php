...
<div id="tribe-events-content" class="upcoming">
	...
	<?php while ( have_posts() ) : the_post(); ?>
		...
		<div id="post-<?php the_ID(); ?>" <?php post_class('tribe-events-event clearfix'); ?> itemscope itemtype="http://schema.org/Event">
			<?php $formate1 =  tribe_get_start_date( null, false, 'Y-m-d h:i:s' ); 
				 $sdate = date('d',strtotime($formate1));
				$smonth = date('F',strtotime($formate1));
				$syear = date('Y',strtotime($formate1));
				$sday = date('l',strtotime($formate1));					
             ?>
             <?php $formate2 =  tribe_get_end_date( null, false, 'Y-m-d h:i:s' ); 
				 $edate  = date('d',strtotime($formate2));
				 $emonth = date('F',strtotime($formate2));
				 $eyear  = date('Y',strtotime($formate2));
				 $eday   = date('l',strtotime($formate2));					
             ?>
...
<div class="date-area m2">
<div class="date"><span><?php echo $sdate; ?> </span><small><?php echo $smonth; ?></small><small class="year"><?php echo $syear; ?></small></div>
<div class="date-cont"><h4><?php the_title('<h2 class="entry-title" itemprop="name"><a href="' . tribe_get_event_link() . '" title="' . the_title_attribute('echo=0') . '" rel="bookmark">', '</a></h2>'); ?></h4>
<?php $venue = tribe_get_venue();
						if ( !empty( $venue ) ) : ?>
   Production: <span> <?php if( class_exists( 'TribeEventsPro' ) ): ?>
  	...
  	<?php endif; ?>
			...
			<?php if( !tribe_is_day() && tribe_is_multiday() ) : ?>
				...
				<div class="date-area m2">
<div class="date"><span><?php echo $sdate; ?> </span><small><?php echo $smonth; ?></small><small class="year"><?php echo $syear; ?></small></div>
<div class="date"><span><?php echo $edate; ?> </span><small><?php echo $emonth; ?></small><small class="year"><?php echo $eyear; ?></small></div>
<div class="date-cont"><h4><?php the_title('<h2 class="entry-title" itemprop="name"><a href="' . tribe_get_event_link() . '" title="' . the_title_attribute('echo=0') . '" rel="bookmark">', '</a></h2>'); ?></h4>
<?php $venue = tribe_get_venue();
						if ( !empty( $venue ) ) : ?>
   Production: <span> <?php if( class_exists( 'TribeEventsPro' ) ): ?>
  	...
  	<?php endif; ?>
			...
			<div class="tribe-events-event-list-meta" itemprop="location" itemscope itemtype="http://schema.org/Place">
				<table cellspacing="0">
					...
					<tr>
						<td class="tribe-events-event-meta-desc"><?php _e('Time:', 'tribe-events-calendar'); ?></td>
						<td class="tribe-events-event-meta-value" itemprop="startDate" content="<?php echo tribe_get_start_date(); ?>">
						<?php echo tribe_get_start_date(); ?><?php // echo date("l",mktime(0,0,0,tribe_get_start_date()));?></td>
					</tr>
					<tr>
						<td class="tribe-events-event-meta-desc"><?php _e('Day:', 'tribe-events-calendar'); ?></td>
						<td class="tribe-events-event-meta-value" itemprop="endDate" content="<?php echo $sday; ?>"><?php echo $sday; ?></td>
					</tr>
					<?php else: ?>
					<tr>
