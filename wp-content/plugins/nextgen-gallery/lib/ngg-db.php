...
//		$order_clause	= "LENGTH(tt.{$order_by}) {$order_dir}, tt.{$order_by} {$order_dir}";
    ...
    */
    ...
    function add_album( $name = false,  $albumDate = '', $previewpic = 0, $description = '', $sortorder = 0, $pageid = 0) {
        global $wpdb;

        // name must be unique, we use the title for that
        $slug = nggdb::get_unique_slug( sanitize_title( $name ), 'album' );

       echo $wpdb->prepare("INSERT INTO $wpdb->nggalbum (name, slug, previewpic, albumdesc, sortorder, added_date, pageid)
													 VALUES (%s, %s, %d, %s, %s, %s, %d)", $name, $slug, $previewpic, $description, $sortorder, $albumDate, $pageid);
		// Add the album
		if ( false === $wpdb->query( $wpdb->prepare("INSERT INTO $wpdb->nggalbum (name, slug, previewpic, albumdesc, sortorder, added_date, pageid)
													 VALUES (%s, %s, %d, %s, %s, %s, %d)", $name, $slug, $previewpic, $description, $sortorder, $albumDate, $pageid) ) ) {
			return false;
		}
