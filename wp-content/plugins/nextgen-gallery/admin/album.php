...
class nggManageAlbum {
	...
	function processor() {
		...
		if ( isset($_POST['add']) && isset ($_POST['newalbum']) ) {
			...
			//echo '<pre>'; print_r($_POST['newalbum']); die();

			if (!nggGallery::current_user_can( 'NextGEN Add/Delete album' ))
				wp_die(__('Cheatin&#8217; uh?'));

			$result = nggdb::add_album( $_POST['newalbum'],$_POST['added_date'] );
	...
	function update_album() {
		...
		$result = $wpdb->query( $wpdb->prepare( "UPDATE $wpdb->nggalbum SET slug= '%s', name= '%s', added_date= '%s', albumdesc= '%s', previewpic= %d, pageid= %d WHERE id = '%d'" , $slug, $name, $addedDate, $desc, $prev, $link, $this->currentID ) );
		...
		<div class="albumnav tablenav">
			<div class="alignleft actions">
				...
				<?php } else { ?>
					...
					<span><?php esc_html_e('Add album date(yyyy-mm-dd)', 'nggallery'); ?>&nbsp;</span>
					<input class="search-input" id="added_date" name="added_date" type="text" value="" />
...
<div id="editalbum" style="display: none;" >
	...
	<table width="100%" border="0" cellspacing="3" cellpadding="3" >
	    ...
	    <tr>
	    	<th>
	    		<?php esc_html_e('Album Date:', 'nggallery'); ?><br />
				<input class="search-input" id="added_date" name="added_date" type="text" value="<?php echo esc_attr( $album->added_date ); ?>" style="width:95%" />
	    	</th>
	   </tr>
	  	<tr>
	    	<th>
